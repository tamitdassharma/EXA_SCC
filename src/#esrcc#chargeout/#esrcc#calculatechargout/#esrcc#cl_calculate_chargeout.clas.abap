CLASS /esrcc/cl_calculate_chargeout DEFINITION
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS: validate_costbase
      ...
      EXPORTING
        !ev_failed           TYPE abap_boolean
        !ev_skip_validations TYPE abap_boolean
      CHANGING
...
* Assign the 16 digit unique identifier
        IF lo_uuid IS BOUND.
          ...
          ENDTRY.
        ENDIF.
*        <ls_cc_cost>-recalrefpoper = lt_keys[ 1 ]-poper.
...
*Delete Recalculations if exist any
    /esrcc/cl_calculate_trueup=>reopen_recalchargeout(
      it_keys          = it_keys
*     it_poper         =
      iv_recalrefpoper = it_keys[ 1 ]-poper
    ).
...
*get the chain and sequence.
    ...
    IF sy-subrc = 0.
      ...
      LOOP AT lt_chain_stw ASSIGNING FIELD-SYMBOL(<ls_chain_stw>).
        ...
        reopen_stdchargeout( it_keys = lt_keys ).
  ...
  METHOD calculate_stdseqchargeout.
...
*********************************************************************
          validate_costbase(
            ...
            IMPORTING
              ...
              ev_skip_validations = DATA(skip_validations)
            CHANGING
              ct_keys   = lt_keys
          ).
...
*    Stop the chain executed and report the errors in log and exit
          ...
          IF skip_validations = abap_false.
...
*    Stop the chain executed and report the errors in log and exit
  ...
  METHOD validate_costbase.
    ...
    CLEAR: ev_skip_validations, ev_failed.
...
*update process control
        ...
        ls_procctrl-process = /esrcc/if_calculate_chargeout=>stdchargeout.    "Cost Base
        ls_procctrl-status  = /esrcc/if_calculate_chargeout=>stdchargeout_failed.     "Cost Base failed
...
*get the list of receivers
      SELECT DISTINCT cb_stw~receiversysid,
            ...
            FROM /esrcc/i_chargeout_receivers AS cb_stw
            INNER JOIN @it_keys AS ik
              ON cb_stw~fplv          = ik~fplv
             AND cb_stw~ryear         = ik~ryear
             AND cb_stw~sysid         = ik~sysid
