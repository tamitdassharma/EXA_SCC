CLASS /esrcc/cl_c_trueup DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS /esrcc/cl_c_trueup IMPLEMENTATION.
...
*
***select data
        DATA lt_result        TYPE STANDARD TABLE OF /esrcc/c_trueup_review.
        DATA lt_fin_result    TYPE STANDARD TABLE OF /esrcc/c_trueup_review.
        DATA ls_result        TYPE /esrcc/c_trueup_review.
...
*check if trigger is via open task My Inbox
        IF _workflowid IS NOT INITIAL.
          SELECT DISTINCT cbstw~* FROM /esrcc/cb_stw AS cbstw
...
*   Get the recalculated chargeouts
        SELECT * FROM /esrcc/i_trup_analysis
           WHERE RefPoper       IN @_refpoper
            ...
            AND  Receivingentity IN @_receivingentity
            AND  ReceiverCompanyCode IN @_recccode
            AND  ReceiverCostObject IN @_reccostobject
            AND  ReceiverCostCenter IN @_reccostcenter
        ...
        IF lt_recalculated IS NOT INITIAL.
...
*      get group config to check the sign of cost
          SELECT SINGLE cost_sign FROM /esrcc/group INTO @DATA(costsign).  "#EC CI_NOWHERE
          ...
          LOOP AT lt_recalculated ASSIGNING FIELD-SYMBOL(<recalculated>).
            ...
            IF ls_result-StdTotalChargeoutAmount <> 0.
              ls_result-DelTotalChargeoutAmount   = ls_result-RecalTotalChargeoutAmount - ls_result-StdTotalChargeoutAmount.
            ELSE.
              ls_result-DelTotalChargeoutAmount   = ls_result-RecalTotalChargeoutAmount - trueupamount.
            ENDIF.
...
*  For the scenario where receivers were changed between standard and chargeout calculations and receivers not found in recalculation
          LOOP AT lt_standard ASSIGNING <standard>.
            ...
            CLEAR ls_result.
            MOVE-CORRESPONDING <standard> TO ls_result.
            ls_result-Refpoper = refpoper.
            ls_result-StdTotalChargeoutAmount   = <standard>-TotalChargeoutAmount + trueupamount.
            ls_result-StdTotalMarkup            = <standard>-TotalMarkup.
            ls_result-StdTotalCostbase          = <standard>-TotalCostbase.
*          ls_result-RecalTotalChargeoutAmount = <recalculated>-TotalChargeoutAmount.
...
*          ls_result-RecalTotalCostbase        = <recalculated>-TotalCostbase.
            ...
            ELSE.
              ls_result-Trueuptype = 'N'.
            ENDIF.
            READ TABLE trueuptypes ASSIGNING <trueuptype> WITH KEY Trueuptype = ls_result-Trueuptype.
            IF sy-subrc = 0.
              ls_result-trueuptypedescription = <trueuptype>-text.
            ENDIF.
            APPEND ls_result TO lt_result.
