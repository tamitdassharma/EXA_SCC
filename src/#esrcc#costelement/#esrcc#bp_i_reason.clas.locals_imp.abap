CLASS lcl_custom_validation DEFINITION.
  PUBLIC SECTION.
    TYPES:
      ...
      tt_reason_create TYPE TABLE FOR CREATE /esrcc/i_reason_s\\reasonall\_reason,
      BEGIN OF ts_control,
    ...
    CLASS-METHODS:
      precheck_cba_reason
        IMPORTING
          entities TYPE tt_reason_create
        CHANGING
          reported TYPE any
          failed   TYPE any.
...
CLASS lhc_rap_tdat_cts IMPLEMENTATION.
  METHOD get.
    result = mbc_cp_api=>rap_tdat_cts( tdat_name = '/ESRCC/REASON'
                                       table_entity_relations = VALUE #( ( entity = 'Reason' table = '/ESRCC/REASON' )
                                                                         ( entity = 'ReasonText' table = '/ESRCC/REASONT' ) ) ) ##NO_TEXT.
  ENDMETHOD.
ENDCLASS.
...
CLASS lhc_/esrcc/i_reason_s IMPLEMENTATION.
  ...
  METHOD get_global_authorizations.
    DATA(is_authorized) = /esrcc/cl_authorization=>check_authorization_tabu( field_name = '/ESRCC/I_REASON' ).
    result-%update      = is_authorized.
    result-%action-edit = is_authorized.
    result-%action-selectcustomizingtransptreq = is_authorized.
  ENDMETHOD.
  METHOD precheck_cba_reason.
    lcl_custom_validation=>precheck_cba_reason(
      EXPORTING
        entities = entities
      CHANGING
        failed   = failed-reason
        reported = reported-reason ).
  ENDMETHOD.
  ...
  PRIVATE SECTION.
    METHODS:
      validatetransportrequest FOR VALIDATE ON SAVE
        IMPORTING
          keys FOR reason~validatetransportrequest,
      get_global_features FOR GLOBAL FEATURES
        IMPORTING
        REQUEST requested_features FOR reason
        RESULT result,
      get_instance_features FOR INSTANCE FEATURES
        IMPORTING keys REQUEST requested_features FOR reason RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR reason RESULT result.

    METHODS copy FOR MODIFY
      IMPORTING keys FOR ACTION reason~copy.
ENDCLASS.
...
CLASS lhc_/esrcc/i_reason IMPLEMENTATION.
  ...
  METHOD copy.
    DATA:
      new_main TYPE TABLE FOR CREATE /esrcc/i_reason_s\_reason,
      new_text TYPE TABLE FOR CREATE /esrcc/i_reason_s\\reason\_reasontext.

    FIELD-SYMBOLS <new_text> LIKE LINE OF new_text.

    IF lines( keys ) > 1.
      INSERT mbc_cp_api=>message( )->get_select_only_one_entry( ) INTO TABLE reported-%other.
      failed-reason = VALUE #( FOR fkey IN keys ( %tky = fkey-%tky ) ).
      RETURN.
    ENDIF.
    ...
    LOOP AT ref_main ASSIGNING FIELD-SYMBOL(<ref_main>).
      ...
      APPEND VALUE #(
        ...
        %target = VALUE #( ( %cid      = key_cid
                             %is_draft = <ref_main>-%is_draft
                             %data     = CORRESPONDING #( <ref_main> EXCEPT reasonid singletonid )
                             reasonid  = key-%param-reasonid ) ) ) TO new_main.

      UNASSIGN <new_text>.
      LOOP AT ref_text ASSIGNING FIELD-SYMBOL(<ref_text>) USING KEY draft WHERE %tky-%is_draft = key-%tky-%is_draft
                                                                            AND %tky-reasonid  = key-%tky-reasonid.
        ...
        INSERT VALUE #( %cid      = key_cid && <ref_text>-spras
                        ...
                        %data     = CORRESPONDING #( <ref_text> EXCEPT reasonid singletonid )
                        reasonid  = key-%param-reasonid ) INTO TABLE <new_text>-%target.
      ENDLOOP.
    ENDLOOP.
...
*   Pre-check validation before create
    ...
    IF failed-reason IS INITIAL.
      MODIFY ENTITIES OF /esrcc/i_reason_s IN LOCAL MODE
        ENTITY reasonall CREATE BY \_reason
        FIELDS (
                 reasonid
                 calculationusage
               ) WITH new_main
        ENTITY reason CREATE BY \_reasontext
        FIELDS (
                 spras
                 reasonid
                 description
    ...
    mapped-reason = mapped_create-reason.
    INSERT LINES OF read_failed-reason INTO TABLE failed-reason.

    IF failed-reason IS INITIAL.
      reported-reason = VALUE #( FOR created IN mapped-reason (
                                     %cid            = created-%cid
                                     %action-copy    = if_abap_behv=>mk-on
                                     %msg            = mbc_cp_api=>message( )->get_item_copied( )
                                     %path-reasonall = VALUE #( %is_draft = created-%is_draft singletonid = 1 ) ) ).
    ENDIF.
  ENDMETHOD.
...
CLASS lhc_/esrcc/i_reasontext DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    METHODS:
      validatetransportrequest FOR VALIDATE ON SAVE
        IMPORTING
          keys FOR reasontext~validatetransportrequest,
      get_global_features FOR GLOBAL FEATURES
        IMPORTING
