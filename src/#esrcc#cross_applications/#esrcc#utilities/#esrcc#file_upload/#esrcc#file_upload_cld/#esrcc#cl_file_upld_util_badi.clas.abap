CLASS /esrcc/cl_file_upld_util_badi DEFINITION PUBLIC FINAL CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      _costs_elements_mapping   IMPORTING from_extraction TYPE xsdboolean
    ...
    DATA:
      ...
      _source_cost_data_sign TYPE /esrcc/sign_for_cogs,
      _user_decimal_format   TYPE xsdboolean,
      _object_types          TYPE _cost_object_formats_type,
...
*                     WHEN 'D' THEN _convert_date_to_internal( EXPORTING date_format = date_format CHANGING value = cell_value ) ).
  ...
  METHOD /esrcc/if_file_upld_util_badi~determine_business_field.
    IF table_name = '/ESRCC/CB_LI_TMP'.
      _costs_elements_mapping( EXPORTING from_extraction = from_extraction CHANGING data_structure = change_structure ).
    ENDIF.
  ENDMETHOD.
...
*       message          =
  ...
  METHOD _costs_elements_mapping.
    ...
    IF _object_types IS INITIAL.
      SELECT FROM /esrcc/cstobjtyp AS object_type
        INNER JOIN /esrcc/cobjtymp AS mapping ON mapping~object_type = object_type~cost_object
        FIELDS object_type~cost_object AS host_object_type, mapping~source_cost_object_type AS source_object_type
        WHERE mapping~active = @abap_true
        INTO TABLE @_object_types.
    ENDIF.

    IF _legal_entity_data IS INITIAL.
      SELECT FROM /esrcc/le_ccode AS legal_entity_master FIELDS sysid, ccode, legalentity WHERE active = @abap_true
    ...
    IF _source_cost_data_sign IS INITIAL.
      SELECT SINGLE FROM /esrcc/sys_info FIELDS sign_for_cogs WHERE system_id = @<system_id> INTO @_source_cost_data_sign.
    ENDIF.
    ASSIGN COMPONENT 'LEGALENTITY' OF STRUCTURE data_structure TO FIELD-SYMBOL(<legal_entity>).
    ASSIGN COMPONENT 'CCODE' OF STRUCTURE data_structure TO FIELD-SYMBOL(<ccode>).
    ASSIGN COMPONENT 'COSTOBJECT' OF STRUCTURE data_structure TO FIELD-SYMBOL(<cost_object>).
...
*      valid_to >= @valid_on INTO @DATA(<mapping_information>).
    LOOP AT _cost_elements_mapping ASSIGNING FIELD-SYMBOL(<mapping_information>) WHERE legalentity = <legal_entity>
                                                                                   AND ccode       = <ccode>
                                                                                   AND costelement = <cost_element>
      ...
      IF <usage_type> IS ASSIGNED.
        <usage_type> = COND #( WHEN <mapping_information>-usagetype IS INITIAL THEN 'E' ELSE <mapping_information>-usagetype ).
        UNASSIGN <usage_type>.
      ENDIF.
    ...
    IF <local_amount> IS ASSIGNED.
      <local_amount> = COND /esrcc/hsl( WHEN _source_cost_data_sign = _group_configuration-cost_sign THEN <local_amount> ELSE <local_amount> * -1 ).
  ...
  METHOD _fetch_from_db.
    ...
    SELECT sysid,
      ...
      ORDER BY sysid, legal_entity, company_code, costelement, valid_from
