CLASS LHC_/ESRCC/I_TPPROF_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE TpProfileAll\_TpProfile
          ENTITIES_UPDATE FOR UPDATE TpProfile.
ENDCLASS.

CLASS LHC_/ESRCC/I_TPPROF_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_TpProf\_TpProfileText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_TpProf\_TpProfileText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_TpProfText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_TpProf\_TpProfileText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Tpprofile = <target>-%KEY-Tpprofile
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_TpProf_S
      ENTITY TpProfile
        CREATE BY \_TpProfileText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_TpProf_S
