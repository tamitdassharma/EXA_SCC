CLASS LHC_/ESRCC/I_LICENSE_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE LicenseAll\_License
          ENTITIES_UPDATE FOR UPDATE License.
ENDCLASS.

CLASS LHC_/ESRCC/I_LICENSE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_License\_LicenseText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_License\_LicenseText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_LicenseText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_License\_LicenseText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-License = <target>-%KEY-License
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_License_S
      ENTITY License
        CREATE BY \_LicenseText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_License_S
