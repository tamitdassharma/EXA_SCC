CLASS LHC_/ESRCC/I_LICTYPE_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE LicenseTypeAll\_LicenseType
          ENTITIES_UPDATE FOR UPDATE LicenseType.
ENDCLASS.

CLASS LHC_/ESRCC/I_LICTYPE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_LicType\_LicenseTypeText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_LicType\_LicenseTypeText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_LicTypeText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_LicType\_LicenseTypeText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-LicenseType = <target>-%KEY-LicenseType
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_LicType_S
      ENTITY LicenseType
        CREATE BY \_LicenseTypeText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_LicType_S
