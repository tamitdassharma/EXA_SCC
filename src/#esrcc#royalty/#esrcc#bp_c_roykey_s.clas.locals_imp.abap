CLASS LHC_/ESRCC/I_ROYKEY_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE RoyKeyAll\_RoyaltyKey
          ENTITIES_UPDATE FOR UPDATE RoyaltyKey.
ENDCLASS.

CLASS LHC_/ESRCC/I_ROYKEY_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_RoyKey\_RoyaltyKeyText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_RoyKey\_RoyaltyKeyText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_RoyKeyText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_RoyKey\_RoyaltyKeyText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-RoyaltyBaseKey = <target>-%KEY-RoyaltyBaseKey
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_RoyKey_S
      ENTITY RoyaltyKey
        CREATE BY \_RoyaltyKeyText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_RoyKey_S
