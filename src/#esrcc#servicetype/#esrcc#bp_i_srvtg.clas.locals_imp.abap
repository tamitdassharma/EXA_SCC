CLASS lcl_custom_validation DEFINITION.
  PUBLIC SECTION.
    TYPES:
      ...
      tt_tg_create TYPE TABLE FOR CREATE /esrcc/i_srvtg_s\\transactiongrpall\_transactiongrp,
      BEGIN OF ts_control,
    ...
    CLASS-METHODS:
      precheck_cba_transaction_group
        IMPORTING
          entities TYPE tt_tg_create
        CHANGING
          reported TYPE any
          failed   TYPE any.
...
CLASS lcl_custom_validation IMPLEMENTATION.
  ...
  METHOD precheck_cba_transaction_group.
...
CLASS lhc_rap_tdat_cts IMPLEMENTATION.
  METHOD get.
    result = mbc_cp_api=>rap_tdat_cts( tdat_name = '/ESRCC/SRVTG'
                                       table_entity_relations = VALUE #( ( entity = 'TransactionGrp' table = '/ESRCC/SRVTG' )
                                                                         ( entity = 'TransactionGrpText' table = '/ESRCC/TGT' ) ) ) ##NO_TEXT.
  ENDMETHOD.
ENDCLASS.
...
CLASS lhc_/esrcc/i_srvtg_s IMPLEMENTATION.
  ...
  METHOD get_global_authorizations.
    DATA(is_authorized) = /esrcc/cl_authorization=>check_authorization_tabu( field_name = '/ESRCC/I_SRVTG' ).
    result-%update      = is_authorized.
    result-%action-edit = is_authorized.
    result-%action-selectcustomizingtransptreq = is_authorized.
  ENDMETHOD.
  METHOD precheck_cba_transactiongrp.
    lcl_custom_validation=>precheck_cba_transaction_group(
      EXPORTING
        entities = entities
      CHANGING
        failed   = failed-transactiongrp
        reported = reported-transactiongrp ).
  ENDMETHOD.
  ...
  PRIVATE SECTION.
    METHODS:
      validatetransportrequest FOR VALIDATE ON SAVE
        IMPORTING
          keys FOR transactiongrp~validatetransportrequest,
      get_global_features FOR GLOBAL FEATURES
        IMPORTING
        REQUEST requested_features FOR transactiongrp
        RESULT result,
      get_instance_features FOR INSTANCE FEATURES
        IMPORTING keys REQUEST requested_features FOR transactiongrp RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR transactiongrp RESULT result.

    METHODS copy FOR MODIFY
      IMPORTING keys FOR ACTION transactiongrp~copy.
ENDCLASS.
...
CLASS lhc_/esrcc/i_srvtg IMPLEMENTATION.
  ...
  METHOD get_global_authorizations.
    result-%action-copy = /esrcc/cl_authorization=>check_authorization_tabu( field_name = '/ESRCC/I_SRVTG' ).
  ENDMETHOD.

  METHOD copy.
    DATA:
      new_main TYPE TABLE FOR CREATE /esrcc/i_srvtg_s\_transactiongrp,
      new_text TYPE TABLE FOR CREATE /esrcc/i_srvtg_s\\transactiongrp\_transactiongrptext.

    FIELD-SYMBOLS <new_text> LIKE LINE OF new_text.

    IF lines( keys ) > 1.
      INSERT mbc_cp_api=>message( )->get_select_only_one_entry( ) INTO TABLE reported-%other.
      failed-transactiongrp = VALUE #( FOR fkey IN keys ( %tky = fkey-%tky ) ).
      RETURN.
    ENDIF.

    READ ENTITIES OF /esrcc/i_srvtg_s IN LOCAL MODE
      ENTITY transactiongrp
      ALL FIELDS WITH CORRESPONDING #( keys )
      RESULT DATA(ref_main)
      FAILED DATA(read_failed).

    READ ENTITIES OF /esrcc/i_srvtg_s IN LOCAL MODE
      ENTITY transactiongrp BY \_transactiongrptext
    ...
    LOOP AT ref_main ASSIGNING FIELD-SYMBOL(<ref_main>).
      ...
      APPEND VALUE #(
        ...
        %target = VALUE #( ( %cid             = key_cid
                             %is_draft        = <ref_main>-%is_draft
                             %data            = CORRESPONDING #( <ref_main> EXCEPT transactiongroup singletonid )
      ...
      LOOP AT ref_text ASSIGNING FIELD-SYMBOL(<ref_text>) USING KEY draft WHERE %tky-%is_draft        = key-%tky-%is_draft
                                                                            AND %tky-transactiongroup = key-%tky-transactiongroup.
        ...
        INSERT VALUE #( %cid             = key_cid && <ref_text>-spras
                        ...
                        %data            = CORRESPONDING #( <ref_text> EXCEPT transactiongroup singletonid )
                        transactiongroup = key-%param-transactiongroup ) INTO TABLE <new_text>-%target.
      ENDLOOP.
    ENDLOOP.
...
*   Pre-check validation before create
    ...
    IF failed-transactiongrp IS INITIAL.
      MODIFY ENTITIES OF /esrcc/i_srvtg_s IN LOCAL MODE
        ENTITY transactiongrpall CREATE BY \_transactiongrp
        FIELDS (
                 transactiongroup
               ) WITH new_main
        ENTITY transactiongrp CREATE BY \_transactiongrptext
        FIELDS (
                 spras
                 transactiongroup
                 description
    ...
    mapped-transactiongrp = mapped_create-transactiongrp.
    INSERT LINES OF read_failed-transactiongrp INTO TABLE failed-transactiongrp.

    IF failed-transactiongrp IS INITIAL AND failed-transactiongrptext IS INITIAL.
      reported-transactiongrp = VALUE #( FOR created IN mapped-transactiongrp (
                                     %cid            = created-%cid
                                     %action-copy    = if_abap_behv=>mk-on
                                     %msg            = mbc_cp_api=>message( )->get_item_copied( )
                                     %path-transactiongrpall = VALUE #( %is_draft = created-%is_draft singletonid = 1 ) ) ).
    ENDIF.
  ENDMETHOD.
...
CLASS lhc_/esrcc/i_srvtgtext DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    METHODS:
      validatetransportrequest FOR VALIDATE ON SAVE
        IMPORTING
          keys FOR transactiongrptext~validatetransportrequest,
      get_global_features FOR GLOBAL FEATURES
        IMPORTING
