CLASS /esrcc/cl_config_util DEFINITION PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      ...
      BEGIN OF ts_royalty_comp_rule_config,
        royalty_comp_method TYPE /esrcc/royaltycomputationmeth,
        value               TYPE abap_boolean,
        amount_value        TYPE abap_boolean,
        currency            TYPE abap_boolean,
      END OF ts_royalty_comp_rule_config,
    ...
    TYPES:
      ...
      tt_royalty_comp_rule_config TYPE STANDARD TABLE OF ts_royalty_comp_rule_config WITH KEY royalty_comp_method,
    ...
    CLASS-METHODS get_royalty_comp_rule_config
      RETURNING
        VALUE(royalty_comp_rule_config) TYPE tt_royalty_comp_rule_config.

    CLASS-METHODS extract_date_components
      IMPORTING
        date  TYPE d
      EXPORTING
        year  TYPE i
    ...
    METHODS validate_start_end_of_month
      ...
      CHANGING
        ...
        !end_date   TYPE /esrcc/validto OPTIONAL.
...
*   Parameters relevant for "Adhoc Chargeout"
  ...
  METHOD get_royalty_comp_rule_config.
*   Parameters relevant for "Fixed Amount"
    APPEND VALUE #( royalty_comp_method = 'FA'
                    value               = abap_false
                    amount_value        = abap_true
                    currency            = abap_true ) TO royalty_comp_rule_config.

*   Parameters relevant for "Fixed Amount per Unit"
    APPEND VALUE #( royalty_comp_method = 'FP'
                    value               = abap_false
                    amount_value        = abap_true
                    currency            = abap_true ) TO royalty_comp_rule_config.

*   Parameters relevant for "Manual Edit Amount"
    APPEND VALUE #( royalty_comp_method = 'ME'
                    value               = abap_false
                    amount_value        = abap_false
                    currency            = abap_false ) TO royalty_comp_rule_config.

*   Parameters relevant for "Percentage Base Value"
    APPEND VALUE #( royalty_comp_method = 'PB'
                    value               = abap_true
                    amount_value        = abap_false
                    currency            = abap_false ) TO royalty_comp_rule_config.
  ENDMETHOD.
  ...
  METHOD validate_initial.
    ...
    LOOP AT fields INTO gs_active_field.
      ...
      IF <value> IS INITIAL OR <value> = ''.
        gv_data_element = /esrcc/cl_abap_dictionary=>get_data_element_by_value( EXPORTING iv_value = <value> ).
        DATA(msg) = NEW cl_abap_behv( )->new_message( id = c_config_msg number = message_no severity = cl_abap_behv=>ms-error v1 = get_field_text( ) ).
      ELSE.
        CLEAR msg.
      ENDIF.
  ...
  METHOD validate_non_mandatory.
    ...
    LOOP AT fields INTO gs_active_field.
      ...
      IF <value> IS NOT INITIAL.
        gv_data_element = /esrcc/cl_abap_dictionary=>get_data_element_by_value( EXPORTING iv_value = <value> ).
        DATA(msg) = NEW cl_abap_behv( )->new_message( id = c_config_msg number = '014' severity = cl_abap_behv=>ms-error v1 = get_field_text( ) v2 = value ).
      ELSE.
        CLEAR msg.
      ENDIF.
...
*   Entity 2
  ...
  METHOD validate_percentage.
    ...
    LOOP AT fields INTO gs_active_field.
      ...
      IF <value> NOT BETWEEN 0 AND 100.
        gv_data_element = /esrcc/cl_abap_dictionary=>get_data_element_by_value( EXPORTING iv_value = <value> ).
        DATA(msg) = NEW cl_abap_behv( )->new_message( id = c_config_msg number = '005' severity = cl_abap_behv=>ms-error v1 = get_field_text( ) ).
      ELSE.
        CLEAR msg.
      ENDIF.
  ...
  METHOD validate_poper.
    ...
    LOOP AT fields INTO gs_active_field.
      ...
      IF <value> NOT BETWEEN '001' AND '012'.
        gv_data_element = /esrcc/cl_abap_dictionary=>get_data_element_by_value( EXPORTING iv_value = <value> ).
        DATA(msg)       = NEW cl_abap_behv( )->new_message( id = c_config_msg number = '004' severity = cl_abap_behv=>ms-error v1 = get_field_text( ) ).
      ELSE.
        CLEAR msg.
      ENDIF.
  ...
  METHOD check_single_input.
    ...
    LOOP AT fields INTO gs_active_field.
      ...
      CASE lv_tabix.
        WHEN 1.
          DATA(v1) = get_field_text( ).
        WHEN 2.
          DATA(v2) = get_field_text( ).
      ENDCASE.
    ...
    IF counter <> 1.
      DATA(msg) = NEW cl_abap_behv( )->new_message( id = c_config_msg number = '032' severity = cl_abap_behv=>ms-error v1 = v1 v2 = v2 ).
    ELSE.
      CLEAR msg.
    ENDIF.
