CLASS /esrcc/cl_badi_wf_adh DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS /esrcc/cl_badi_wf_adh IMPLEMENTATION.
...
*
*      CLEAR lv_text_concat.
*      lv_text_concat =   |<h3>Adhoc Charge-Out Details</h3> | .
*      APPEND lv_text_concat TO et_task_desc.
*
*      CLEAR lv_text_concat.
*      lv_text_concat =   '<table>' .
...
*      lv_text_concat =   |<tr> <td>Total Charge-Out Amount</td> <td> { lv_price_str  } | & | { ls_rec_cost_sum-currency } </td> </tr>| .
*      APPEND lv_text_concat TO et_task_desc.
*
*      CLEAR lv_text_concat.
...
*Cost Center || 2021/M/March || SERV || 007735
      CONCATENATE 'Ad-hoc Charge-out Request ||' lv_legal_rec '||'  lv_billing_period '||'  lv_legal_s '||'  lv_srv_prod_s
                        INTO  ev_header SEPARATED BY space .
...
*    SORT lt_leading_object_inp BY fplv ryear ccode costobject legalentity costobject costcenter.
**     serviceproduct receivingentity recsysid recccode reccostobject reccostcenter.
*
*    DELETE ADJACENT DUPLICATES FROM lt_leading_object_inp COMPARING fplv ryear ccode costobject legalentity costobject costcenter.
**     serviceproduct receivingentity recsysid recccode reccostobject reccostcenter.

    SORT lt_leading_object_inp BY fplv ryear ccode costobject costcenter legalentity serviceproduct receivingentity recsysid recccode reccostobject reccostcenter.
