CLASS /esrcc/cl_badi_wf_scm DEFINITION
  PUBLIC
  CREATE PUBLIC .

  PUBLIC SECTION.

    INTERFACES if_badi_interface .
    INTERFACES /esrcc/if_workflow_enh .
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.



CLASS /ESRCC/CL_BADI_WF_SCM IMPLEMENTATION.


  METHOD /esrcc/if_workflow_enh~get_agents.
    /esrcc/cl_wf_agents=>get_agents(
     EXPORTING
       iv_wf_id          = iv_wf_id
       iv_approval_level = iv_approval_level
     IMPORTING
       et_agents         = et_agents
   ).
  ENDMETHOD.


  METHOD /esrcc/if_workflow_enh~set_wf_task_description.
*cds Name   /ESRCC/I_SRVMKUP_COST

    CLEAR: et_task_desc.
    DATA lv_price                  TYPE c LENGTH 50.
    DATA lv_price_str              TYPE string.
    DATA lv_text_concat            TYPE string.
    DATA lv_service_product_list   TYPE string.
    DATA lv_service_product_temp   TYPE string.
    DATA lv_cost_decimal_2         TYPE p  DECIMALS 2.

    APPEND '<h3>General Information</h3>' TO et_task_desc.
** Keys for CDS
**  Fplv,Ryear, Poper,Sysid, Legalentity, Ccode,Costobject,Costcenter,Serviceproduct

    SELECT fplv,ryear,poper,sysid,legalentity,ccode,costobject,costcenter,
      serviceproduct,costbase~currency,
      srvcostshare,
      valueaddshare,
      passthroughshare,
*      totalsrvmarkupabs,
*      valueaddmarkupabs,
*      passthrumarkupabs,

*      totalchargeoutamount,
      legalentitydescription,
      ccodedescription,
      costobjectdescription,
      costcenterdescription,
      costdatasetdescription,
*      billingfrequencydescription,
*      billingperioddescription,
      serviceproductdescription
      FROM /ESRCC/I_CostBaseStewardship AS costbase
           INNER JOIN /ESRCC/I_ServiceProductShare AS serviceproductshare
            ON costbase~uuid = serviceproductshare~ParentUUID
            AND costbase~Currencytype = serviceproductshare~Currencytype
      FOR ALL ENTRIES IN @it_leading_object WHERE
*                                                   fplv = @it_leading_object-fplv
*                                              AND  ryear = @it_leading_object-ryear
*                                              AND  poper = @it_leading_object-poper
*                                              AND  legalentity = @it_leading_object-legalentity
*                                              AND  ccode = @it_leading_object-ccode
*                                              AND  costobject = @it_leading_object-costobject
*                                              AND  costcenter = @it_leading_object-costcenter
*                                              AND  billingfrequqncy = @it_leading_object-billfrequency
*                                              AND  billingperiod = @it_leading_object-billingperiod
                                                   costbase~uuid = @it_leading_object-cc_uuid
                                              AND  serviceproduct = @it_leading_object-serviceproduct
                                              AND  costbase~currencytype = 'L'

      INTO  TABLE @DATA(lt_srvmkup) .

** Get Service Product
    SELECT DISTINCT serviceproduct ,serviceproductdescription FROM @lt_srvmkup AS srvmkup INTO TABLE @DATA(lt_service_product_list).

*    LOOP AT lt_service_product_list INTO DATA(ls_srv_sp).
*      IF sy-tabix = 1.
*        lv_service_product_list = | { ls_srv_sp-serviceproduct } ( { ls_srv_sp-serviceproductdescription } )  | .
*        CONTINUE.
*      ELSE.
*        lv_service_product_temp = | { ls_srv_sp-serviceproduct } ( { ls_srv_sp-serviceproductdescription } )  | .
*      ENDIF.
*
*      CONCATENATE lv_service_product_list lv_service_product_temp INTO lv_service_product_list
*      SEPARATED BY ','.
*
*    ENDLOOP.
** Do sum
    SELECT fplv,ryear, poper, sysid,legalentity,ccode,costobject,costcenter,currency,
         SUM( srvcostshare ) AS srvcostshareg  ,
         SUM( valueaddshare ) AS valueaddshareg  ,
         SUM( passthroughshare ) AS passthroughshareg  ,
*         SUM( totalsrvmarkupabs ) AS totalsrvmarkupabsg,

*        SUM( valueaddmarkupabs ) AS valueaddmarkupabs,
*        SUM( passthrumarkupabs ) AS passthrumarkupabs,
*        SUM( totalsrvmarkupabs ) AS totalsrvmarkupabs,
*        SUM( totalchargeoutamount )  AS totalchargeoutamount ,
         legalentitydescription,
         ccodedescription,
         costobjectdescription,
         costcenterdescription,
         costdatasetdescription
*         billingfrequencydescription,
*         billingperioddescription
         FROM @lt_srvmkup AS cc_cost
         GROUP BY fplv,ryear,poper,sysid,legalentity,ccode,costobject,
         costcenter,currency,
         legalentitydescription,
         ccodedescription,
         costobjectdescription,
         costcenterdescription,
         costdatasetdescription
*         billingfrequencydescription,
*         billingperioddescription
        INTO TABLE @DATA(lt_srv_mkp_sum).


    LOOP AT lt_srv_mkp_sum INTO DATA(ls_srv_mkp_sum).

      CLEAR lv_text_concat.
      lv_text_concat =   '<table>' .
      APPEND lv_text_concat TO et_task_desc.

      CLEAR lv_text_concat.
      lv_text_concat =   |<tr> <td>Cost Dataset</td> <td>{ ls_srv_mkp_sum-costdatasetdescription } </td></tr>| .
      APPEND lv_text_concat TO et_task_desc.


      CLEAR lv_text_concat.
      lv_text_concat =   |<tr> <td>Year </td> <td> { ls_srv_mkp_sum-ryear } </td></tr> | .
      APPEND lv_text_concat TO et_task_desc.

      CLEAR lv_text_concat.
      lv_text_concat =   |<tr><td>Period </td> <td> { ls_srv_mkp_sum-poper }</td></tr> | .
      APPEND lv_text_concat TO et_task_desc.

*      CLEAR lv_text_concat.
*      lv_text_concat =   |<tr><td>Billing Frequency </td> <td> { ls_srv_mkp_sum-billingfrequencydescription }</td></tr> | .
*      APPEND lv_text_concat TO et_task_desc.
*
*
*      CLEAR lv_text_concat.
*      lv_text_concat =   |<tr><td>Billing Period</td> <td> { ls_srv_mkp_sum-billingperioddescription }</td></tr> | .
*      APPEND lv_text_concat TO et_task_desc.



      CLEAR lv_text_concat.
      lv_text_concat =   |<tr> <td>System Id (Source) </td> <td> { ls_srv_mkp_sum-sysid } </td></tr>| .
      APPEND lv_text_concat TO et_task_desc.


      CLEAR lv_text_concat.
      lv_text_concat =   |<tr><td>Legal Entity </td> <td>  { ls_srv_mkp_sum-legalentity } ({ ls_srv_mkp_sum-legalentitydescription })</td></tr> | .
      APPEND lv_text_concat TO et_task_desc.

      CLEAR lv_text_concat.
      lv_text_concat =   |<tr> <td>Company Code</td> <td>  { ls_srv_mkp_sum-ccode } ({ ls_srv_mkp_sum-ccodedescription })</td></tr> | .
      APPEND lv_text_concat TO et_task_desc.




      CLEAR lv_text_concat.
      lv_text_concat =   |<tr><td>Cost Object Type (Source) </td> <td>  { ls_srv_mkp_sum-costobjectdescription }</td></tr> | .
      APPEND lv_text_concat TO et_task_desc.


      CLEAR lv_text_concat.
      lv_text_concat =   |<tr><td>Cost Object Number (Source) </td> <td>  { ls_srv_mkp_sum-costcenter } ({ ls_srv_mkp_sum-costcenterdescription })</td></tr> | .
      APPEND lv_text_concat TO et_task_desc.

      LOOP AT lt_service_product_list INTO DATA(ls_srv_sp).
*      IF sy-tabix = 1.
        lv_service_product_list = | { ls_srv_sp-serviceproduct } ( { ls_srv_sp-serviceproductdescription } )  | .
*        CONTINUE.
*      ELSE.
*        lv_service_product_temp = | { ls_srv_sp-serviceproduct } ( { ls_srv_sp-serviceproductdescription } )  | .
*      ENDIF.

*        CONCATENATE lv_service_product_list lv_service_product_temp INTO lv_service_product_list
*        SEPARATED BY ','.


        CLEAR lv_text_concat.
        lv_text_concat =   |<tr><td>Service Product </td> <td>  { lv_service_product_list }</td></tr> | .
        APPEND lv_text_concat TO et_task_desc.

      ENDLOOP.

      CLEAR lv_text_concat.
      lv_text_concat =   '</table>' .
      APPEND lv_text_concat TO et_task_desc.



      CLEAR lv_text_concat.
      lv_text_concat =   |<h3>Service Cost Share Details</h3>| .
      APPEND lv_text_concat TO et_task_desc.



      CLEAR lv_text_concat.
      lv_text_concat =   '<table>' .
      APPEND lv_text_concat TO et_task_desc.


      CLEAR lv_text_concat.
* Data from CDS is coming in DEC23 with 5 decimal place.
* Write with currency doesnt work with 5 decimal place, thats why we are changing the value to decimal place 2
      lv_cost_decimal_2 = ls_srv_mkp_sum-srvcostshareg.
      lv_price = |{ lv_cost_decimal_2 CURRENCY = ls_srv_mkp_sum-currency }|.
      lv_price_str  = lv_price.
      lv_text_concat =   |<tr> <td>Total Service Cost </td> <td> { lv_price_str  } | & | { ls_srv_mkp_sum-currency }</td></tr> | .
      APPEND lv_text_concat TO et_task_desc.


      CLEAR lv_text_concat.
      lv_cost_decimal_2 =  ls_srv_mkp_sum-valueaddshareg.
      lv_price = |{ lv_cost_decimal_2 CURRENCY = ls_srv_mkp_sum-currency }|.
      lv_price_str  = lv_price.
      lv_text_concat =   |<tr> <td>Thereof Value-Add</td> <td> { lv_price_str  }  | & | { ls_srv_mkp_sum-currency }</td></tr>  | .
      APPEND lv_text_concat TO et_task_desc.


      CLEAR lv_text_concat.
      lv_cost_decimal_2 = ls_srv_mkp_sum-passthroughshareg.
      lv_price = |{ lv_cost_decimal_2 CURRENCY = ls_srv_mkp_sum-currency }|.
      lv_price_str  = lv_price.
      lv_text_concat =   |<tr> <td> Thereof Pass-Through </td> <td> { lv_price_str }  | & | { ls_srv_mkp_sum-currency } </td></tr>| .
      APPEND lv_text_concat TO et_task_desc.

*      CLEAR lv_text_concat.
*      lv_cost_decimal_2 = ls_srv_mkp_sum-valueaddmarkupabs.
*      WRITE  lv_cost_decimal_2  TO lv_price CURRENCY ls_srv_mkp_sum-currency.
*      lv_price_str  = lv_price.
*      lv_text_concat =   |<tr> <td> Total Mark-up on Value-Add </td> <td> { lv_price_str }  | & | { ls_srv_mkp_sum-currency } </td></tr>| .
*      APPEND lv_text_concat TO et_task_desc.
*
*      CLEAR lv_text_concat.
*      lv_cost_decimal_2 = ls_srv_mkp_sum-passthrumarkupabs.
*      WRITE lv_cost_decimal_2 TO lv_price CURRENCY ls_srv_mkp_sum-currency.
*      lv_price_str  = lv_price.
*      lv_text_concat =   |<tr> <td> Total Mark-up Pass-Through </td> <td> { lv_price_str }  | & | { ls_srv_mkp_sum-currency } </td></tr>| .
*      APPEND lv_text_concat TO et_task_desc.
*
*      CLEAR lv_text_concat.
*      lv_cost_decimal_2 = ls_srv_mkp_sum-totalsrvmarkupabs.
*      WRITE lv_cost_decimal_2  TO lv_price CURRENCY ls_srv_mkp_sum-currency.
*      lv_price_str  = lv_price.
*      lv_text_concat =   |<tr> <td>  Total Mark-up </td> <td> { lv_price_str  }  | & | { ls_srv_mkp_sum-currency } </td></tr>| .
*      APPEND lv_text_concat TO et_task_desc.
*
*      CLEAR lv_text_concat.
*      lv_cost_decimal_2 =  ls_srv_mkp_sum-totalchargeoutamount.
*      WRITE lv_cost_decimal_2 TO lv_price CURRENCY ls_srv_mkp_sum-currency.
*      lv_price_str  = lv_price.
*      lv_text_concat =   |<tr> <td> Total Charge-Out Amount </td> <td> { lv_price_str }  | & | { ls_srv_mkp_sum-currency } </td></tr>| .
*      APPEND lv_text_concat TO et_task_desc.

      CLEAR lv_text_concat.
      lv_text_concat =   '<table>' .
      APPEND lv_text_concat TO et_task_desc.


    ENDLOOP.

  ENDMETHOD.


  METHOD /esrcc/if_workflow_enh~set_wf_task_title.
*cds Name   /ESRCC/I_SRVMKUP_COST

    CLEAR: ev_header.


    READ TABLE it_leading_object INTO DATA(ls_leading_object) INDEX 1.
    IF sy-subrc EQ 0.

*Get Legal Entity Description
      SELECT SINGLE legalentity, description FROM /esrcc/i_legalentityall_f4
         WHERE legalentity = @ls_leading_object-legalentity INTO @DATA(ls_le).

*Get Cost objects
      SELECT SINGLE costobject,text FROM /esrcc/i_costobjects
        WHERE costobject =  @ls_leading_object-costobject INTO @DATA(ls_costobject).

*Get Cost Center Description
      SELECT SINGLE sysid,costcenter,costobject,description FROM /esrcc/i_coscen_f4
          WHERE costcenter =  @ls_leading_object-costcenter
        AND sysid =  @ls_leading_object-sysid
        AND costobject = @ls_leading_object-costobject
        INTO @DATA(ls_cost_center).

**Get Description Billing period
*      SELECT SINGLE billingperiod, text FROM /esrcc/i_billingperiod WHERE billingperiod = @ls_leading_object-billingperiod INTO @DATA(ls_billing_period).
*
**Get Description Billing  frequency
*      SELECT SINGLE billingfreq, text FROM /esrcc/i_billingfreq WHERE billingfreq =  @ls_leading_object-billfrequency INTO @DATA(ls_billing_frequency).

      DATA(lv_legal_s) =   |{ ls_leading_object-legalentity }| & | ({ ls_le-description })|.
      DATA(lv_costobject_s) =   |{ ls_costobject-text }|.
      DATA(lv_costcenter_s) =   |{ ls_leading_object-costcenter }| & | ({ ls_cost_center-description })|.
*      DATA(lv_billing_period) =   |{ ls_leading_object-ryear }/| & |{ ls_billing_frequency-text }/| & |{ ls_billing_period-text }| .
      DATA(lv_billing_period) =   |{ ls_leading_object-ryear }/| & |{ ls_leading_object-poper }| .
* Description should be like
**Cost Center || 2021/M/March || SERV || 007735
*      CONCATENATE lv_costobject_s  '||'  lv_billing_period '||'  lv_legal_s '||'  lv_costcenter_s
*                         INTO  ev_header SEPARATED BY space .
*SERV || 2021/M/March ||  007735
      CONCATENATE lv_legal_s '||'  lv_billing_period '||'  lv_costcenter_s
                         INTO  ev_header SEPARATED BY space .


    ENDIF.
  ENDMETHOD.


  METHOD /esrcc/if_workflow_enh~split_and_create_wf_data.
* Group based on Legal Entity and Cost Center
    DATA lt_leading_object_inp  TYPE /esrcc/tt_wf_leadingobject.
    DATA lt_leading_object_out  TYPE /esrcc/tt_wf_leadingobject.
    DATA ls_out_split_data         TYPE  /esrcc/s_split_workflow.

    lt_leading_object_inp = it_leading_object.

    SORT lt_leading_object_inp BY fplv ryear legalentity ccode costobject costcenter.

    DELETE ADJACENT DUPLICATES FROM lt_leading_object_inp COMPARING fplv ryear legalentity ccode costobject costcenter.

    LOOP AT lt_leading_object_inp INTO DATA(ls_leading_object_cb_inp).
      CLEAR lt_leading_object_out.
      CLEAR ls_out_split_data.

      LOOP AT it_leading_object INTO DATA(ls_leading_object)
        WHERE
        fplv = ls_leading_object_cb_inp-fplv
        AND ryear = ls_leading_object_cb_inp-ryear
        AND legalentity = ls_leading_object_cb_inp-legalentity
        AND ccode = ls_leading_object_cb_inp-ccode
        AND costobject = ls_leading_object_cb_inp-costobject
        AND costcenter  = ls_leading_object_cb_inp-costcenter.

        APPEND ls_leading_object TO lt_leading_object_out.
      ENDLOOP.

      IF lt_leading_object_out IS NOT INITIAL.
        APPEND LINES OF lt_leading_object_out TO ls_out_split_data-segment.
        APPEND ls_out_split_data TO et_wf_split_data.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
ENDCLASS.
